...
from llm.vllm import invoke_openai_api
...
def get_clean_string(s):
    s = str(s).lower().strip()
    for suffix in ["meters", "meter", "mm", "m", "mile", "miles", "thousand", "million", "billion", "kg", "acres",
                   "minutes"]:
...
def robust_literal_eval(pred):
    # 基本校验：如果不是字符串或不是以 [ 开头，直接返回
    if not isinstance(pred, str) or not pred.strip().startswith("["):
    ...
    # 2. 尝试正则提取带引号的内容 (处理 "years' imprisonment" 情况)
    # 找被引号包裹，且后面跟着 , 或 ] 的内容
    matches = re.findall(r"['\"](.*?)['\"](?=\s*,\s*|\s*\])", pred)
    if matches:
        return matches

    # 3. 终极兜底 (处理 "[Tables 12 and 13]" 这种完全没引号的情况)
    # 直接剥离最外层的 []，并返回剩余内容的列表
    content = pred.lstrip('[').rstrip(']')
    if content:
        # 如果中间没有逗号，说明就是一个整体，直接返回
        if ',' not in content:
            return [content.strip()]
        # 如果有逗号，尝试按逗号分割（这可能不完美，但在格式混乱时最有效）
        return [item.strip().strip("'\"") for item in content.split(',')]

    raise 'robust_literal_eval'
    return []
        ...
        try:
            if len(gt) == len(pred):
                if len(gt) == 0:
                    ...
                    element_scores = [
                        answer_score(a, b, threshold=0.8) if not (isfloat(a) or is_format_match(a)) else a == b for a, b
                        in zip(gt, pred)]
...
def eval_samples(samples, dataset_name: str):
    ...
    if dataset_name == "MMLong":
        ...
        try:
            recall = sum(
                sample["score"]["Acc"] for sample in evaluated_samples if sample["answer"] != "Not answerable") / len(
                [sample for sample in evaluated_samples if sample["answer"] != "Not answerable"])
            precision = sum(
                sample["score"]["Acc"] for sample in evaluated_samples if sample["answer"] != "Not answerable") / len(
                [sample for sample in evaluated_samples if sample["pred_ans"] != "Not answerable"])

            f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0
        except:
            f1 = 0.0
            if metrics['Acc'] == metrics['EM']:
                f1 = metrics['Acc'] / 100
...
def extract_answer(question, output, extractor_prompt, model_name):
    try:
        full_query_prompt = f"{extractor_prompt}\n\nQuestion: {question}\nAnalysis: {output}\n"
        response = invoke_openai_api(model_name=model_name, content=full_query_prompt)
    except Exception as e:
...
def extract_score(question, output, ground_truth, prompt, model_name):
    try:
        query_prompt = prompt.format(question=question, answer=output, gt=ground_truth)
        eval_str = invoke_openai_api(model_name=model_name, content=query_prompt)
...
def show_fine_grained_results(samples, dataset="MMLong"):
    ...
    multi_pages_results = eval_samples(
        [sample for sample in samples if len(sample["evidence_pages"]) > 1 and sample["answer"] != "Not answerable"],
        dataset)
    ...
    all_results = {
        "Overall_score": score_dict,
        "single_page": single_page_results,
        "multi_pages": multi_pages_results,
    ...
    for source, source_samples in source_sample_dict.items():
        ...
        all_results[source] = cur_results
